...
# MODULE 9: STREAMLIT
# BACKEND FASTAPI - SECURITY ENABLED (SERVES THE ANALYSIS TO THE FRONT END) 
# ----


# To Run this App:
# - Open Terminal
# - uvicorn 09_streamlit.zzz_02_fastapi_app_security:app --reload --port 8000
...
async def main():
    ...
    content = """
    <body>
    <h1>Welcome to the Email Lead Scoring Project (With Security Enabled)</h1>
...
@app.get("/get_email_subscribers", dependencies=[Depends(get_api_key)] )
...
@app.post("/data", dependencies=[Depends(get_api_key)])
...
#calculate_lead_strategy(Request, monthly_sales_reduction_safe_guard=0.9, email_list_size =100000, unsub_rate_per_sales_email = 0.005, sales_emails_per_month = 5, avg_sales_per_month = 250000.0, avg_sales_emails_per_month =5, customer_conversion_rate =0.05, avg_customer_value = 2000.0)
