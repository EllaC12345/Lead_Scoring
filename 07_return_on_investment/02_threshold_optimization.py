...
def lead_strategy_calc_expected_value(
    ...
    # 4.4 Expected Saved Customers (Unrealized until nurture takes effect)
...
lead_make_strategy(leads_scored_df, thresh=0.90, verbose = True) \
    .pipe(lead_aggregate_strategy_results) \
...
thresh_optim = lead_select_optimum_thresh(thresh_optim_df, monthly_sales_reduction_safe_gaurd = 0.90, verbose = True)
...
lead_get_expected_value(
    thresh_optim_df,
    threshold = lead_select_optimum_thresh(thresh_optim_df, monthly_sales_reduction_safe_gaurd = 0.90),
    verbose = True
)

#%%
# 7.0 PLOT THE OPTIMAL THRESHOLD ----
#unstyle the thresh_optim_df before  
thresh_optim_df = thresh_optim_df.data
thresh_optim_df
